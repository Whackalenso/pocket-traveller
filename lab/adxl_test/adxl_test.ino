// tamagotchi.ino — Pico + ADXL345 (I2C 0x1D) + SH1106 SPI OLED
//
// ADXL345 notes:
//   * Address 0x1D = SDO tied HIGH. (SDO tied LOW would be 0x53.)
//   * DEVID register 0x00 must read 0xE5. If it doesn't, you are not
//     talking to a real ADXL345.
//   * The part boots in STANDBY. POWER_CTL (0x2D) bit 3 must be set or the
//     data registers return valid-looking but frozen values.
//   * Full-resolution mode is 3.9 mg/LSB and RIGHT-justified, so there is
//     NO >>4 shift (that was a LIS3DH quirk). 1 g reads ~256 counts.
//     We scale counts -> milli-g below so 1 g = 1000, matching the units
//     the old LIS3DH path produced. That keeps ENERGY_GATE tuning valid.
//
...
uint8_t readReg(uint8_t reg) {
  Wire.beginTransmission(ADDR);
  Wire.write(reg);
  Wire.endTransmission();          // full stop (more reliable than repeated start here)
  Wire.requestFrom(ADDR, (uint8_t)1);
  return Wire.available() ? Wire.read() : 0;
}

// ADXL345 full-resolution scale factor: 3.9 mg per LSB.
// Multiplying by this puts magnitude in milli-g, so 1 g = 1000 — the same
// units the old LIS3DH (>>4, 1 mg/LSB) path produced.
static const float LSB_TO_MG = 3.9f;

// Read all three axes in one burst starting at DATAX0 (0x32).
...
void enableNormalMode() {
  writeReg(0x31, 0x08);   // DATA_FORMAT: full resolution, +/-2g
...
void setup() {
  ...
  // Sanity check: a real ADXL345 returns 0xE5 from DEVID (0x00).
...
void loop() {
  ...
  if ((long)(now - nextT) >= 0) {
    ...
    int16_t x, y, z;
    readXYZ(x, y, z);   // raw counts, right-justified, no shift
    // Magnitude in milli-g so 1 g = 1000 (keeps ENERGY_GATE tuning valid).
    pushSample(sqrtf((float)x * x + (float)y * y + (float)z * z) * LSB_TO_MG);
    sinceHop++;
